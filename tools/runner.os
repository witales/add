...
Процедура ВывестиСправку()
	...
	Сообщить("  Запуск тестирования vanessa-add");
	Сообщить("    inputPath путь к каталогу или к файлу с тестами");
	Сообщить("    outputPath путь к каталогу с отчетами");
	Сообщить("    --pathvanessa [env RUNNER_PATHVANESSA] путь к внешней обработке, по умолчанию vendor/add/bddRunner.epf");
...
//  ПутьКИнструментам - <Строка> - путь к инструментам, по умолчанию ./vendor/add
...
Процедура ЗапуститьТестироватьПоведение(Знач РабочийКаталогПроекта = "./build/out",
	...
	Если ПустаяСтрока(ПутьКИнструментам) Тогда
		ПутьКИнструментам = "./add/bddRunner.epf";
	КонецЕсли;
...
Процедура Декомпилировать(Знач Путь, Знач КаталогВыгрузки, Знач КорневойПутьВыходной = "", Знач КорневойПутьВходной = "",
	...
	НастройкиИзФайла = ПрочитатьНастройкиФайлJSON(КорневойПутьПроекта, "./build/cache.json");
		...
		Если ЕстьИзмененияДляЗаписи Тогда
			ЗаписатьФайлJSON(ОбъединитьПути(КорневойПутьПроекта, "./build/cache.json"), НастройкиИзФайла);
		КонецЕсли;
		Возврат ;
	//Исключение
...
Процедура РазобратьКаталог(Знач ОбъектКаталога, Знач КаталогВыгрузки, Знач КаталогКорень, Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы="") Экспорт
	...
	Для Каждого Файл из Файлы Цикл
		Если Лев(Файл.Имя,4) = ".git" Или Врег(Файл.Имя) = Врег("build") Или Врег(Файл.Имя) = Врег("cache.json") Тогда
			Продолжить;
		КонецЕсли;
...
Функция Компилировать(Знач Путь, Знач КаталогВыгрузки, Знач СтрокаПодключения = "", Знач Пользователь = "",
	...
	КлючКэша = ?(КлючКэша = Неопределено ИЛИ Не ЗначениеЗаполнено(КлючКэша), "compileepfrunner", КлючКэша);
		...
		ЕстьИзмененияДляЗаписи = Ложь;
		Лог.Отладка("Начали собирать inDir %1, outDir %2", ПутьКИсходникам, КаталогВыгрузки);
...
Процедура ПодготовитьЛоги()
	...
	Лог_cmdline.УстановитьУровень(Лог.Уровень());
	Лог_v8runner.УстановитьУровень(Лог.Уровень());
	...
	Попытка
		...
		Если Команда = ВозможныеКоманды().ДекомпилироватьEpf Тогда
			Декомпилировать(ПолныйПуть(Аргументы.ЗначенияПараметров["inputPath"]), ПолныйПуть(Аргументы.ЗначенияПараметров["outputPath"]), "", "",
							...
							Аргументы.ЗначенияПараметров["--v8version"], Аргументы.ЗначенияПараметров["--cachekey"]);
		ИначеЕсли Команда = ВозможныеКоманды().СобратьEpf Тогда
			Компилировать(ПолныйПуть(Аргументы.ЗначенияПараметров["inputPath"]), ПолныйПуть(Аргументы.ЗначенияПараметров["outputPath"]),
							Аргументы.ЗначенияПараметров["--ibname"], Аргументы.ЗначенияПараметров["--db-user"], Аргументы.ЗначенияПараметров["--db-pwd"],
							Аргументы.ЗначенияПараметров["--v8version"], Аргументы.ЗначенияПараметров["--onlycopy"], Аргументы.ЗначенияПараметров["--cachekey"]);
