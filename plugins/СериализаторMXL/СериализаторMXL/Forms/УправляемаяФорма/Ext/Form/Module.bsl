...
Процедура ПротестироватьЗагрузкуМакета(Команда)
	...
	Если Элементы.ГруппаМакеты.ТекущаяСтраница = Элементы.ГруппаТекстовыйДокумент Тогда
...
Процедура СохранитьМакетДанныхВФайл(Команда)
	...
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		ОбработчикОповещения = Новый ОписаниеОповещения("ОбработчикЗаписиМакетаВФайл", ЭтотОбъект);
...
Функция ПолучитьРезультатСсылкамиОтбораНаСервере(ОтборКомпоновщика,Знач ИмяМетаданных,Знач ИмяТипаМетаданных)
	...
	//Добавим выбранные поля к нашему отчету (Эти поля будут наследоваться автополем в группировках ниже)
...
&НаСервере
Процедура ЗагрузитьДанныеМакетаВБазуНаСервере(Макет)
...
Процедура ЗагрузитьДанныеМакетаВБазу(Команда)
	...
	Если Элементы.ГруппаМакеты.ТекущаяСтраница = Элементы.ГруппаТекстовыйДокумент Тогда
		МакетДляЗагрузки = МакетТекстовый;
	КонецЕсли;
	ЗагрузитьДанныеМакетаВБазуНаСервере(МакетДляЗагрузки);
КонецПроцедуры
